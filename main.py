Dentro de python de la maquina virtual api
...
def count():
    conn = get_connection()
    cursor = conn.cursor()
    print(cursor.execute("select count(*) from usuarios u INNER JOIN tarjetas t on u.usuario_id = t.usuario_id WHERE t.estado = 'Activa'"))
    total2 = cursor.fetchone()[0]
    cursor.close()
    conn.close()
    return {"users_active_count":total2}
...
def count():
    conn = get_connection()
    cursor = conn.cursor()
    print(cursor.execute("SELECT usuario_id AS id, CONCAT(nombre, ' ', apellido) AS full_name FROM USUARIOS ORDER BY fecha_registro DESC LIMIT 1;"))
    total3 = cursor.fetchone()[0]
    cursor.close()
    conn.close()
    return {"latest_user":total3}
